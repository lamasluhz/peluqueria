...
const CortesRow = ({ corte, handleFieldUpdate, handleDeleteCorte }) => {
  const [editing, setEditing] = useState(false);
  const [editedFields, setEditedFields] = useState({
    id: corte.id,
    tipo: corte.tipo,
    descripcion: corte.descripcion,
    decMonto: corte.decMonto,
    eliminado: corte.eliminado
  });
  ...
  const handleSaveEdit = () => {
    handleFieldUpdate(corte.id, editedFields);
    setEditing(false);
  };
