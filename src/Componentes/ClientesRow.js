  ...
  const fetchDetallesTurno = async (id) => {
    try {
      const response = await axios.get(
        `https://localhost:7137/api/Factura/serviciosPorCliente/${id}`
      );
  ...
  return (
    <tr id={cliente.id}>
      ...
      <td>
        {editing ? (
          <>
            <i className="fa-solid fa-check" onClick={handleSaveEdit}></i>
            <i className="fa-solid fa-times" onClick={handleCancelEdit}></i>
          </>
        ) : (
          <i
